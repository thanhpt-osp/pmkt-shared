...
| **TD-03** | Port assignment thực tế: `audit-notification = 8084`, `integration = 8085` (swap so với footnote plan §6). Documentation chưa cập nhật. | Phase 2.4 Batch 2 | Low — chỉ doc, code đã đúng. | **Cổng cuối** (B5) khi viết deployment doc. | 🟡 **Partial 2026-05-20** — B4 Phase 4.3 ApplicationSet + Phase 4.4 Kong routes confirm port matrix; doc tổng hợp B5. |
...
| **TD-09** | Secret manager integration test chưa có. Rule §7 #7 sẽ ra ở T3.8 — chỉ doc, impl defer. | Rule §7 #7 | Medium — Spring `application.yml` chứa placeholder, prod cần secret manager. | **B5 (T4.11)**. | 🔴 Open — B4 hardcode dev password 5 component (Postgres + Redis + MinIO + Grafana + Schema Registry); chi tiết docs/db-bootstrap.md + redis-minio.md + observability.md. |
