...
| **TD-05** | springdoc-openapi mới có dependency, chưa enable `@OpenAPIDefinition` + `@Tag` + schema gen. Rule §7 #8 chỉ ProblemDetail partial. | Cổng 1 + 2 review | Medium — API contract chưa machine-validated. | **B5** khi service có Controller business. | ✅ **Resolved 2026-05-21 (B5 Phase 5.6)** — 6 service `OpenApiConfig.java` + `@OpenAPIDefinition` + `SecurityScheme bearerAuth` + `springdoc:` block ở application.yml + `/api-docs` + `/swagger-ui.html` permit (dev). `DanhMucDoiTacController` annotate showcase `@Tag` + `@Operation` + `@ApiResponses`. Prod tắt qua `springdoc.api-docs.enabled=false`. |
| **TD-06** | `CleanArchTest` ArchUnit chưa viết — Rule §7 #1 entry chỉ có scope-guard + Modulith verify. Layer dependency rule (api→app→domain) chưa enforce. | Cổng 1 review | Medium — code có thể vi phạm Clean Arch không bị catch. | **B5** khi template module mở rộng (≥2 module). | ✅ **Resolved 2026-05-21 (B5 Phase 5.6)** — `pmkt-shared/.../archtest/AbstractCleanArchTest.java` abstract base với 4 ArchUnit rule (layered architecture + 3 domain dependency restriction). 6 service subclass `CleanArchTest extends AbstractCleanArchTest` override `basePackage()`. Empty service skeleton B4 pass 4/4 (optionalLayer + allowEmptyShould). Auto-enforce khi layer code xuất hiện Phase 2. |
